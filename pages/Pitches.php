    ...
    <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          ...
          <?php
          if (!$isSignedIn) {
            echo ' <li><a class="get-a-quote" href="./SignUp.php">Register</a></li>          <li><a class="cta-btn" href="SignIn.php">Login</a></li>
            ';
          } else {
            echo '
        ...
        <form method="post" id="" action="search.php">
          <div class="row">
            ...
            <div class="col-9">
              ...
              <input type="text" class="form-control " placeholder="Enter Pitch name or location" name="search">
            </div>
            <div class="col-3">
...
';
  ...
  <input type="text" hidden name="ID" id="ID" value="' . $row['id'] . '">
    ...
    ';
                  while ($t2 = mysqli_fetch_array($RentingSlots)) {
                    ...
                    $reservedRentingSlots = "SELECT * FROM reservation WHERE renting_slot_id = '$t2[id]' AND (statues = 'confirmed' OR statues = 'pending')";
                    $reservedRentingSlotsresult = mysqli_query($link, $reservedRentingSlots);

                    if (mysqli_fetch_array($reservedRentingSlotsresult)) {
                      echo '
                    <input type="radio" disabled class="btn-check" name="choosen" id="option' . $t2['id'] . '" autocomplete="off" value="' . $t2['id'] . '">
                    <label class="btn btn-outline-danger" for="option' . $t2['id'] . '">
 ...
 <div class="modal-footer">
  ...
  <div data-bs-toggle="modal" data-bs-target="#pitch' . $row['id'] . '" class="service-work card border-0 text-white shadow-sm overflow-hidden mx-5 m-sm-0">
      <div class="service">';
                  echo '<img class="card-img " src="../assets/img/new-header.jpg" alt="Card image cap">';
                  // while ($tn = mysqli_fetch_array($photos)) {
                  //   if ($tn)
                  //     echo '<img class="card-img " src="../controllers/image/' . $tn['filename'] . '" alt="Card image cap">';
                  //   else
                  //     echo '<img class="card-img " src="../assets/img/new-header.jpg" alt="Card image cap">';
                  // }
                
                  // $photos2Query = "SELECT * FROM photos WHERE stadium_id = '" . $row['id'] . "' LIMIT 1";
                  // $photos2Result = mysqli_query($link, $photosQuery);
                  // if ($photosResult) {
                  //   $photoRow = mysqli_fetch_array($photosResult);
                  //   if ($photoRow) {
                  //     // Set the image source to the first photo's path
                  //     echo '<img src="../controllers/image/' . $photoRow['filename'] . '" alt="Stadium Image" />';
                  //   }
                  // }
                  echo '
      </div>
